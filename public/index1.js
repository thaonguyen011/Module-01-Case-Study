...
// Phai truyen gia tri cho tham so
// onload image??
class Player {
  constructor() {
    this.velocity = {
...
class Boss {
  constructor() {
    ...
    const image = new Image();
    image.src = "./Image/Mob_Giant_Monster_Gloom.webp";
    image.onload = () => {
      const scale = 0.25;
      this.image = image;
      this.width = image.width * scale;
...
function animate() {
  if (game.active) {
    ...
    if (countGrid === 3 && grids.length === 0) {
      boss.update();
      setInterval(() => {
        boss.shoot(bossProjectiles);

        bossProjectiles.forEach((bossProjectile, i) => {
          if (
          ...
          } else bossProjectile.update();

          if (
            bossProjectile.position.y + bossProjectile.height >=
              player.position.y &&
            bossProjectile.position.y <= player.position.y + player.height &&
    ...
    if (frames % randomInterval === 0 && countGrid < 3) {
      grids.push(new Grid());
